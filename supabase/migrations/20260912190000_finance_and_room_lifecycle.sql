...
create index if not exists financial_transactions_outlet_idx on public.financial_transactions(outlet);
create index if not exists financial_transactions_booking_idx on public.financial_transactions(booking_id);
...
create index if not exists financial_expenses_incurred_at_idx on public.financial_expenses(incurred_at desc);

alter table public.financial_transactions enable row level security;
alter table public.financial_expenses enable row level security;
drop policy if exists financial_transactions_staff_read on public.financial_transactions;
create policy financial_transactions_staff_read on public.financial_transactions for select using (public.is_staff(auth.uid()));
drop policy if exists financial_expenses_staff_read on public.financial_expenses;
create policy financial_expenses_staff_read on public.financial_expenses for select using (public.is_staff(auth.uid()));
drop policy if exists financial_expenses_staff_insert on public.financial_expenses;
create policy financial_expenses_staff_insert on public.financial_expenses for insert with check (public.is_staff(auth.uid()));
drop policy if exists financial_expenses_staff_update on public.financial_expenses;
create policy financial_expenses_staff_update on public.financial_expenses for update using (public.is_staff(auth.uid())) with check (public.is_staff(auth.uid()));

create or replace function public.record_financial_transaction(
  p_transaction_type text, p_direction text, p_amount numeric, p_source_type text, p_source_id text,
  p_description text, p_booking_id text default null, p_customer_id text default null,
  p_outlet text default null, p_method text default null, p_reference text default null,
  p_occurred_at timestamptz default now(), p_created_by uuid default null,
...
begin
  insert into public.financial_transactions (
    transaction_type, direction, amount, source_type, source_id, booking_id, customer_id,
    outlet, method, reference, description, metadata, occurred_at, created_by
  ) values (
    p_transaction_type, p_direction, p_amount, p_source_type, p_source_id, p_booking_id, p_customer_id,
    p_outlet, p_method, p_reference, p_description, coalesce(p_metadata, '{}'::jsonb),
    coalesce(p_occurred_at, now()), p_created_by
  ...
  if v_id is null then
    ...
    where source_type=p_source_type and source_id=p_source_id
      and transaction_type=p_transaction_type and direction=p_direction limit 1;
  end if;
  return v_id;
end;
$$;
...
as $$
declare v_booking_id text; v_customer_id text; v_occurred_at timestamptz;
begin
  if new.status not in ('success','refunded') then return new; end if;
  select b.id, b.customer_id into v_booking_id, v_customer_id from public.bookings b where b.reference=new.booking_ref limit 1;
  v_customer_id := coalesce(new.customer_id, v_customer_id);
  v_occurred_at := coalesce(new.date::timestamptz, now());
  perform public.record_financial_transaction('sale','credit',new.amount,'payment',new.id,
    'Payment received for booking '||new.booking_ref,v_booking_id,v_customer_id,'rooms',new.method,new.reference,v_occurred_at,null,
    jsonb_build_object('payment_id',new.id,'booking_ref',new.booking_ref));
  if new.status='refunded' then
    perform public.record_financial_transaction('refund','debit',new.amount,'payment_refund',new.id,
      'Refund issued for payment '||new.reference,v_booking_id,v_customer_id,'rooms',new.method,new.reference,now(),null,
      jsonb_build_object('payment_id',new.id,'booking_ref',new.booking_ref));
  end if;
  return new;
end;
...
begin
  if new.status='posted' then
    perform public.record_financial_transaction('expense','debit',new.amount,'expense',new.id,new.description,
...
-- Backfill existing posted payment history into the ledger.
insert into public.financial_transactions (
  transaction_type,direction,amount,source_type,source_id,booking_id,customer_id,outlet,method,reference,description,metadata,occurred_at
)
select
  'sale','credit',p.amount,'payment',p.id,b.id,p.customer_id,'rooms',p.method,p.reference,
  'Payment received for booking '||p.booking_ref,
  jsonb_build_object('payment_id',p.id,'booking_ref',p.booking_ref),p.date::timestamptz
from public.payments p
left join public.bookings b on b.reference=p.booking_ref
...
select
  'refund','debit',p.amount,'payment_refund',p.id,b.id,p.customer_id,'rooms',p.method,p.reference,
  'Refund issued for payment '||p.reference,
  jsonb_build_object('payment_id',p.id,'booking_ref',p.booking_ref),now()
from public.payments p
left join public.bookings b on b.reference=p.booking_ref
...
create unique index if not exists housekeeping_tasks_one_active_room_idx on public.housekeeping_tasks(room_id)
...
begin
  ...
  if new.status='checked_in' and (old.status is distinct from new.status or old.room_id is distinct from new.room_id) then
    ...
    select r.room_number,rt.name into v_room_number,v_room_type from public.rooms r left join public.room_types rt on rt.id=r.room_type_id where r.id=new.room_id;
...
values
  ('manager','finance','Finance',true,240),('accountant','finance','Finance',true,240),
  ('reception','finance','Finance',false,240),('housekeeping','finance','Finance',false,240),
  ('maintenance','finance','Finance',false,240),('restaurant','finance','Finance',false,240),('bar','finance','Finance',false,240)
