...
-- 1. A pending/unpaid booking is NOT a sold room.
-- 2. Only a successful paid reservation blocks a room as sold.
-- 3. The separate payment_holds table protects a physical room while a guest
--    is actively inside Paystack's payment window.
-- 4. Non-overlapping dates remain independently bookable.
...
begin
  ...
  -- Pending/unpaid bookings are intentionally ignored here. They are not
  -- proof that the room was sold. The payment_holds table is responsible for
  -- the short payment race window, and only paid bookings permanently block
