...
begin
  ...
  if p_room_id is not null then
    ...
    if r.status <> 'available' then raise exception 'ROOM_NOT_AVAILABLE'; end if;
  else
    select * into r
    from public.rooms x
  ...
  if exists (
    ...
    where bx.room_id = r.id
      and bx.id is distinct from null
      and (
        (bx.status in ('confirmed','checked_in') and bx.payment_status = 'paid')
        or
        (bx.status = 'pending' and bx.payment_status <> 'paid'
...
exception
  when exclusion_violation then raise exception 'ROOM_SOLD';
end;
$$;
