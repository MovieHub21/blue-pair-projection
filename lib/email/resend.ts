...
export async function sendResendEmail(input: { to: string; subject: string; html: string; text?: string }) {
  const response = await fetch(RESEND_API_URL, { method: 'POST', headers: { Authorization: `Bearer ${requiredEnv('RESEND_API_KEY')}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: requiredEnv('RESEND_FROM_EMAIL'), to: [input.to], subject: input.subject, html: addBrandLogo(input.html), ...(input.text ? { text: input.text } : {}) }), cache: 'no-store' })
