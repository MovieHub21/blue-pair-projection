...
function addBookingAccountCta(html: string, text: string | undefined, subject: string) {
  if (!subject.toLowerCase().includes('payment confirmed')) return { html, text }
...
export async function sendResendEmail(input: { to: string; subject: string; html: string; text?: string }) {
  const bookingAccountCta = addBookingAccountCta(input.html, input.text, input.subject)
