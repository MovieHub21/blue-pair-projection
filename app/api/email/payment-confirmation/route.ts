...
export async function POST(request: Request) {
  try {
    ...
    const { data: payment, error: paymentError } = await admin.from('payments').select('id,booking_ref,amount,customer_id,status').eq('reference', String(reference)).maybeSingle()
    ...
    const { data: existing } = await admin.from('guest_notifications').select('id').eq('user_id', user.id).eq('type', 'payment').contains('metadata', { payment_reference: String(reference) }).limit(1).maybeSingle()
    if (existing) return NextResponse.json({ ok: true, alreadySent: true })
    const { data: booking } = await admin.from('bookings').select('id,reference,room_type_id,check_in,check_out,amount').eq('reference', payment.booking_ref).maybeSingle()
