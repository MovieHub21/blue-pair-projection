...
import {
  ...
  serviceRequestStatusEmail, supportTicketEmail,
...
export async function POST(request: Request) {
  try {
    ...
    switch (event) {
      ...
      case 'support_acknowledged':
        if (!requestRow) return NextResponse.json({ error: 'Request not found' }, { status: 404 })
        email = supportTicketEmail({ guestName, reference: requestRow.id, subject: requestRow.type, message: requestRow.message, status: 'received' }); break
      case 'support_status':
        if (!requestRow) return NextResponse.json({ error: 'Request not found' }, { status: 404 })
        email = supportTicketEmail({ guestName, reference: requestRow.id, subject: requestRow.type, message: requestRow.message, status: requestRow.status }); break
