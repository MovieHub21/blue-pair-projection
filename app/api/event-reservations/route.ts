...
export async function POST(request: Request) {
  try {
    const body=await readSanitizedJson<any>(request); const eventId=String(body.eventId||''); const guestName=String(body.guestName||''); const guestEmail=String(body.guestEmail||'').toLowerCase(); const guestPhone=String(body.guestPhone||''); const guestCount=Math.max(1,Number(body.guestCount||1)); const notes=String(body.notes||'')
