...
export async function POST(request: Request) {
  try {
    ...
    if (!secret) {
      await admin.from('bar_order_checkouts').delete().eq('reference', checkoutReference)
    ...
    const paystackResponse = await fetch('https://api.paystack.co/transaction/initialize', {
      ...
      body: JSON.stringify({
        email: customer.email || contactEmail,
    ...
    if (!paystackResponse.ok || !result?.status || !result?.data?.authorization_url) {
      await admin.from('bar_order_checkouts').update({ status: 'failed' }).eq('reference', checkoutReference)
      return NextResponse.json({ error: result?.message || 'Paystack could not initialize payment.' }, { status: 502 })
    }
    await admin.from('bar_order_checkouts').update({ payment_reference: result.data.reference }).eq('reference', checkoutReference)
    return NextResponse.json({ authorizationUrl: result.data.authorization_url, reference: result.data.reference })
  } catch (error:any) {
    console.error('[annex-order-payment-init]', error)
