...
export async function POST(request: Request) {
  try {
    ...
    const checkoutReference='BARPAY-'+Date.now()+'-'+Math.random().toString(36).slice(2,8).toUpperCase(); const payload={items:normalized,location,bookingId:resolvedBookingId,deliveryLabel,notes,contactEmail,contactPhone,deliveryAddress}
