...
export async function GET(request: Request) {
  ...
  const { data, error } = await server.from('guest_notifications').select('id,type,title,body,href,metadata,read_at,created_at').eq('user_id', user.id).order('created_at', { ascending: false }).limit(limit)
  if (error) return NextResponse.json({ error: 'Unable to load notifications.' }, { status: 500 })

  const { count } = await server.from('guest_notifications').select('id', { count: 'exact', head: true }).eq('user_id', user.id).is('read_at', null)
