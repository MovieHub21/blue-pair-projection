...
export async function POST(request: Request) {
  try {
    ...
    if (!booking.room_id) return NextResponse.json({ error: 'This booking does not have a physical room assigned yet.' }, { status: 409 })
    ...
    try {
      ...
      await admin.rpc('release_payment_lock', { p_booking_id: booking.id })
      throw error
    }
  } catch (error: any) {
