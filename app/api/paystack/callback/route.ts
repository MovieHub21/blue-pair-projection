...
export async function GET(request: Request) {
  ...
  try {
    ...
    const { data: booking, error: bookingLookupError } = await admin.from('bookings')
    ...
    const ownsActiveLock = room?.payment_lock_booking_id === booking.id
    ...
    const { error: bookingUpdateError } = await admin.from('bookings')
    ...
    const { error: lockClearError } = await admin.from('rooms')
      .update({ payment_lock_booking_id: null, payment_lock_expires_at: null })
      .eq('id', booking.room_id)
      .eq('payment_lock_booking_id', booking.id)
    if (lockClearError) throw lockClearError

    const { data: competitors, error: competitorError } = await admin.from('bookings')
    ...
    const losers = (competitors ?? [])
      .filter(item => overlaps(booking.check_in, booking.check_out, item.check_in, item.check_out))
      .map(item => item.id)
    if (losers.length) {
      ...
      if (!existingNotification) {
        ...
        if (customer.email) {
          try {
            ...
            console.error('[paystack-callback] confirmation email failed', emailError)
          }
        }
        ...
        await admin.from('guest_notifications').insert({
          ...
          metadata: { booking_id: booking.id, reference: booking.reference, payment_reference: reference, status: 'confirmed' },
        })
      }
    }
