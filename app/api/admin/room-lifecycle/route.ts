...
async function requireStaff() {
  ...
  const { data: staff } = await admin.from('staff').select('id,status,role').eq('user_id', user.id).maybeSingle()
...
async function sendToDepartment(department: 'housekeeping' | 'maintenance', subject: string, html: string, text: string) {
  ...
  const recipients = (staff ?? [])
    .filter(s => {
      const value = `${s.department ?? ''} ${s.role ?? ''}`.toLowerCase()
      return value.includes(department)
    })
    .map(s => String(s.email || '').trim().toLowerCase())
    .filter(Boolean)

  await Promise.all(recipients.map(to => sendResendEmail({ to, subject, html, text }).catch(error => {
    console.error(`[room-lifecycle] ${department} notification failed`, error)
    return null
  })))
...
async function notifyReadyReservations(admin: ReturnType<typeof createSupabaseAdminClient>, roomId: string) {
  ...
  for (const booking of pending ?? []) {
    ...
    const { data: blocking } = await admin.from('bookings')
    ...
    const html = `<div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;color:#0a1229"><h2>Your room is ready to secure</h2><p>Hello ${name},</p><p>Room ${roomNumber} at Blue Pair Hotel is now available for your stay from <strong>${escapeHtml(booking.check_in)}</strong> to <strong>${escapeHtml(booking.check_out)}</strong>.</p><p>Your reservation is still unpaid, so it does not hold the room exclusively. <strong>The first successful payment secures the room.</strong></p><p><a href="${paymentUrl}" style="display:inline-block;background:#c79a3e;color:#0a1229;text-decoration:none;padding:12px 18px;border-radius:8px;font-weight:700">Open my booking &amp; pay</a></p><p style="font-size:13px;color:#667085">Reservation reference: ${reference} · Room ${roomNumber}</p></div>`
    ...
    if (logError) { console.error('[room-lifecycle] reservation-ready log failed', logError.message); continue }
    sent++
    ...
    if (customer?.user_id) {
      await admin.from('guest_notifications').insert({
        ...
        title: 'Your reserved room is available',
        body: `Room ${room.room_number} is now available. Pay to secure your reservation before another guest does.`,
...
export async function POST(request: Request) {
  try {
    ...
    if (action === 'maintenance_request') {
      ...
      const room = escapeHtml(requestRow.room || '—')
      const guest = escapeHtml(requestRow.guest_name || 'Guest')
      const message = escapeHtml(requestRow.message || 'No additional details.')
    ...
    if (action === 'status') {
      ...
      if (body.status === 'cleaning' || body.status === 'cleaning_required') {
        ...
        if (!existing) {
          ...
          await admin.from('housekeeping_tasks').insert({
            ...
            checkout_time: new Date().toLocaleTimeString('en-NG', { hour: '2-digit', minute: '2-digit' }),
            priority: body.status === 'cleaning_required' ? 'High' : 'Normal',
            assigned_to: 'Unassigned',
            status: body.status === 'cleaning' ? 'in_progress' : 'pending',
            notes: body.status === 'cleaning_required' ? 'Room requires cleaning after status change.' : 'Room manually marked for cleaning.',
            room_id: room.id,
          })
        }
      ...
      if (body.status === 'maintenance') {
        notified = await sendToDepartment('maintenance', `Maintenance required — Room ${room.room_number}`, `<div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;color:#0a1229"><h2>Room sent to maintenance</h2><p>Room <strong>${escapeHtml(room.room_number)}</strong> has been marked <strong>maintenance</strong>.</p><p>Please inspect the room and create or update the maintenance ticket with the issue and resolution.</p></div>`, `Room ${room.room_number} has been marked maintenance. Please inspect it and update the maintenance ticket.`)
      }
      if (body.status === 'available') {
        const ready = await notifyReadyReservations(admin, room.id)
        return NextResponse.json({ ok: true, status: body.status, notified, reservationsReady: ready })
      }
    ...
    if (booking.room_id) {
      ...
      const notified = await sendToDepartment('housekeeping', `Guest checkout — Room ${room?.room_number || '—'} requires cleaning`, `<div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;color:#0a1229"><h2>Guest checkout</h2><p>Room <strong>${escapeHtml(room?.room_number || '—')}</strong> has just been checked out.</p><p><strong>Guest:</strong> ${escapeHtml(customer?.name || 'Guest')}</p><p><strong>Booking:</strong> ${escapeHtml(booking.reference)}</p><p>The room is now marked <strong>Cleaning Required</strong>.</p></div>`, `Guest checkout. Room ${room?.room_number || '—'} requires cleaning. Guest: ${customer?.name || 'Guest'}. Booking: ${booking.reference}.`)
