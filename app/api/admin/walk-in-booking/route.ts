...
export async function POST(request: Request) {
  try {
    ...
    const extraServices = Array.isArray(body.extraServices)
    ...
    const hasConflict = (conflictingBookings ?? []).some((booking: any) => checkIn < String(booking.check_out) && String(booking.check_in) < checkOut)
    if (hasConflict) return NextResponse.json({ error: `Room ${selectedRoom.room_number} is already reserved for some of those dates.` }, { status: 409 })
    ...
    const extrasTotal = extraServices.reduce((sum: number, service: any) => sum + service.price, 0)
    const tax = Math.round((subtotal + extrasTotal) * 0.075)
    const amount = subtotal + extrasTotal + tax

    let customerId: string
    const { data: existingCustomer, error: customerLookupError } = await admin.from('customers').select('id').ilike('email', email).maybeSingle()
    ...
    if (existingCustomer) {
      ...
      const { error } = await admin.from('customers').insert({ id: customerId, user_id: null, name, email, phone, status: 'active' })
      if (error) throw error
    }
    ...
    return NextResponse.json({ booking, customer: { id: customerId, name, email, phone }, room: { id: selectedRoom.id, roomNumber: selectedRoom.room_number }, totals: { nights, subtotal, extrasTotal, tax, amount } }, { status: 201 })
  } catch (error: any) {
