...
export async function GET() {
  try {
    ...
    const { data, error } = await admin.from('guest_reviews').select('id,rating,title,review,keywords,created_at,updated_at,published,booking_id').eq('user_id', user.id).order('created_at', { ascending: false }).limit(1).maybeSingle()
...
export async function POST(request: Request) {
  try {
    ...
    const { data: eligibleBookings, error: bookingError } = await admin.from('bookings').select('id,checked_in_at,check_in,check_out,status').eq('customer_id', customer.id).or('checked_in_at.not.is.null,status.in.(checked_in,checked_out)').order('checked_in_at', { ascending: false, nullsFirst: false }).order('check_in', { ascending: false })
    ...
    const { data: created, error } = await admin.from('guest_reviews').insert({ user_id: user.id, booking_id: eligible.id, rating: input.rating, title: input.title || null, review: input.review, keywords: input.keywords, published: true, published_at: new Date().toISOString() }).select('id,rating,title,review,keywords,created_at,updated_at,published,booking_id').single()
    if (error) throw error
...
export async function PUT(request: Request) {
  try {
    ...
    const body = await readSanitizedJson<any>(request)
    ...
    const admin = createSupabaseAdminClient()
    const { data: existing, error: findError } = await admin.from('guest_reviews').select('id').eq('id', reviewId).eq('user_id', user.id).maybeSingle()
    if (findError) throw findError
    if (!existing) return NextResponse.json({ error: 'Review not found or you do not have permission to edit it.' }, { status: 404 })

    const { data: updated, error } = await admin.from('guest_reviews').update({ rating: input.rating, title: input.title || null, review: input.review, keywords: input.keywords, updated_at: new Date().toISOString(), published: true, published_at: new Date().toISOString() }).eq('id', reviewId).eq('user_id', user.id).select('id,rating,title,review,keywords,created_at,updated_at,published,booking_id').single()
    if (error) throw error
