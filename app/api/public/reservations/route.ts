...
export async function POST(request: Request) {
  try {
    ...
    const { data: existingReservations } = await db.from('bookings').select('id,reference,room_id,check_in,check_out').eq('customer_id', customerId).eq('room_type_id', roomTypeId).eq('status', 'pending').neq('payment_status', 'paid')
    const duplicate = (existingReservations ?? []).find(b => (!roomId || b.room_id === roomId) && overlaps(checkIn, checkOut, b.check_in, b.check_out))
    if (duplicate) return NextResponse.json({ booking: duplicate, alreadyReserved: true }, { status: 200 })
    ...
    const { data: booking, error: bookingError } = await db.from('bookings').insert({ id: `b_${Date.now()}`, reference, customer_id: customerId, room_type_id: roomTypeId, room_id: selectedRoomId, check_in: checkIn, check_out: checkOut, adults: Number(adults) || 2, children: Number(children) || 0, amount: Number(amount) || Number(roomType.price) * Math.max(1, Math.round((new Date(checkOut).getTime() - new Date(checkIn).getTime()) / 86400000)), payment_status: 'pending', status: 'pending' }).select('id,reference,room_id').single()
