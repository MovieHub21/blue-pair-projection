...
export async function GET(request: Request) {
  try {
    ...
    for (const room of result) { const row=byType[room.room_type_id]??{available:0,availableSoon:0,taken:0,reserved:0,pending:0,earliestAvailable:null}; if(room.guest_status==='available')row.available++;else if(room.guest_status==='availableSoon')row.availableSoon++;else if(room.guest_status==='reserved')row.reserved++;else row.taken++;if(room.pending)row.pending++;if(room.available_from&&(!row.earliestAvailable||room.available_from<row.earliestAvailable))row.earliestAvailable=room.available_from;byType[room.room_type_id]=row }
