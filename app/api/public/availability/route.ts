...
function guestStatus(room: any, bookings: any[], checkIn: string, checkOut: string) {
  const overlappingPaid = bookings.filter(paidReservation).filter(b => bookingOverlaps(b, checkIn, checkOut))
  ...
  // Only the explicit admin "Available Soon" state is exposed as a customer-facing inventory block.
  // Cleaning, Cleaning Required and Maintenance remain operational/admin states and do not block booking here.
  if (room.status === 'available_soon') return { status: 'availableSoon', availableFrom: null }
  if (room.status === 'available' || room.status === 'cleaning' || room.status === 'cleaning_required' || room.status === 'maintenance') return { status: 'available', availableFrom: checkIn }
