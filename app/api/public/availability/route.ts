...
export async function GET(request:Request){try{
 ...
 const result=(rooms??[]).map(room=>{
  ...
  return {...room,payment_lock_booking_id:undefined,payment_lock_expires_at:room.payment_lock_expires_at,guest_status:ownReservation?'reserved':guestStatus,admin_status:adminStatus,availability_reason:reason,available_from:availableFrom,payment_ready:false,payment_locked:Boolean(room.payment_lock_expires_at&&new Date(room.payment_lock_expires_at).getTime()>Date.now()),payment_locked_by_me:false,pending:pending.length>0,pending_count:pending.length,reservation_id:ownReservation?.id,reservation_expires_at:ownReservation?.reservation_expires_at,payment_locked_by_me:!!ownReservation}
 })
