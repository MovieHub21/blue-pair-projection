...
export default function GlobalError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error('Blue Pair global application error', { digest: error?.digest })
  }, [error])

  return (
    <html lang="en-NG">
      <body className="bg-[#070d1a]">
        <main className="min-h-screen bg-[#070d1a] px-5 py-12 text-white flex items-center justify-center">
          <section className="w-full max-w-xl text-center">
            ...
            <h1 className="font-serif text-4xl font-semibold tracking-tight sm:text-5xl">We’re temporarily unavailable</h1>
            <p className="mx-auto mt-4 max-w-md text-sm leading-7 text-white/60 sm:text-base">
              Something unexpected happened. Please try again or return to the Blue Pair home page.
            </p>
