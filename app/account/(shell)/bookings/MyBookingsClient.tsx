...
type BookingWithRoom = Booking & { room: RoomType | null; roomNumber?: string | null; checkedInAt?: string; checkedOutAt?: string }
    ...
    const checkReadiness = async () => {
      ...
      const results = await Promise.all(pending.map(async b => {
        try {
          ...
          return [b.id, { ready: Boolean(room?.payment_ready), checking: false, locked: Boolean(room?.payment_locked), lockedByMe: Boolean(room?.payment_locked_by_me), lockExpiresAt: room?.payment_lock_expires_at ?? null }] as const
  ...
  async function payForBooking(booking: BookingWithRoom) {
    ...
    try {
      ...
      if (!response.ok || !data.authorizationUrl) {
        if (data.code === 'PAYMENT_IN_PROGRESS') { setBlockedUntil(prev => ({ ...prev, [booking.id]: data.expiresAt || null })); setPaymentError('Another guest is currently completing payment for this room. Your booking is still active, and you can try again when their 10-minute payment window ends.') }
        else if (data.code === 'ROOM_SOLD') setPaymentError('This room has just been secured by another guest. Your booking can no longer be paid for this room.')
      ...
      {bookings.map(b => {
        ...
        return <div key={b.id} className="card p-5 flex flex-col sm:flex-row gap-4 sm:items-center">
          ...
          <div className="flex-1">
            <div className="flex justify-between items-start gap-2 flex-wrap"><div><b className="block">{b.room?.name ?? 'Room'}{b.roomNumber ? ` · Room ${b.roomNumber}` : ''}</b><span className="text-xs text-navy-400">{b.reference} · Reservation: {formatDate(b.checkIn)} → {formatDate(b.checkOut)}</span></div><div className="text-right"><b className="font-display block">{naira(b.amount)}</b><StatusBadge status={b.paymentStatus} /></div></div>
