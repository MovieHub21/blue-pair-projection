        ...
        return <div key={b.id} className="card p-5 flex flex-col sm:flex-row gap-4 sm:items-center">
          ...
          <div className="flex-1">
            <div className="flex justify-between items-start gap-2 flex-wrap">
              <div><b className="block">{b.room?.name ?? 'Room'}{b.roomNumber ? ` · Room ${b.roomNumber}` : ''}</b><span className="text-xs text-navy-400">{b.reference} · {formatDate(b.checkIn)} → {formatDate(b.checkOut)}</span></div>
      ...
      {active && (() => {
        ...
        return <div className="flex flex-col gap-0.5">
          {[
            ...
            ['Check-in', formatDate(active.checkIn)], ['Check-out', formatDate(active.checkOut)],
            ['Guests', `${active.adults} adults, ${active.children} children`], ['Amount', naira(active.amount)],
            ['Payment status', active.paymentStatus], ['Booking status', active.status], ['Special requests', active.specialRequests || 'None'],
