...
export default function AmenityManagementPage({ params }: { params: { key: string } }) {
  ...
  useEffect(() => { if (amenity) setDraft({ description: amenity.description, hours: amenity.hours, pricingNote: amenity.pricingNote, facilities: amenity.facilities.join(', ') }) }, [amenity?.key, amenity?.description, amenity?.hours, amenity?.pricingNote, amenity?.facilities.join(',')])
  if (!amenity) return <p className="text-sm text-navy-400">{loaded ? 'This section was not found.' : 'Loading…'}</p>
  ...
  return (
    <div>
      ...
      <div className="card p-6 max-w-2xl flex flex-col gap-5">
        ...
        <div><label className="field-label">Main (hero) photo</label><div className="flex items-center gap-4"><div className="w-40 h-24 rounded-lg overflow-hidden bg-cream-100">{amenity.heroImage && <img src={amenity.heroImage} className="w-full h-full object-cover" alt={amenity.name} />}</div><ImageUploader folder={`amenities/${k}`} label="Upload from device" onUploaded={async urls => { if (await saveAmenity(k, { heroImage: urls[0] })) pushToast('Main photo updated', 'success'); else pushToast('Could not save the main photo. Please try again.', 'error') }} /></div></div>
        <div><label className="field-label">Gallery photos</label><div className="grid grid-cols-4 gap-2.5 mb-3">{gallery.map((src, i) => <div key={src + i} className="relative aspect-square rounded-lg overflow-hidden group"><img src={src} className="w-full h-full object-cover" alt={`${amenity.name} photo ${i + 1}`} /><button onClick={() => setDeleteIndex(i)} aria-label={`Delete ${amenity.name} photo ${i + 1}`} className="absolute top-1 right-1 w-7 h-7 rounded-full bg-red-600/90 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 group-focus-within:opacity-100"><Trash2 size={12} /></button></div>)}</div><ImageUploader folder={`amenities/${k}`} multiple label="Upload photos from device" onUploaded={async urls => { if (await saveAmenity(k, { gallery: [...gallery, ...urls] })) pushToast('Photos added', 'success'); else pushToast('Could not save the photos. Please try again.', 'error') }} /></div>
        <div className="flex gap-3 pt-2"><button onClick={async () => { const saved = await saveAmenity(k, { description: draft.description, hours: draft.hours, pricingNote: draft.pricingNote, facilities: draft.facilities.split(',').map(f => f.trim()).filter(Boolean), published: true }); pushToast(saved ? `${amenity.name} page published` : `Could not publish the ${amenity.name} page. Please try again.`, saved ? 'success' : 'error') }} className="btn-primary">Publish changes</button><button onClick={async () => { if (await saveAmenity(k, { published: false })) pushToast('Saved as draft', 'info'); else pushToast('Could not save the draft. Please try again.', 'error') }} className="btn-outline">Save as draft</button></div>
      </div>
      <DeleteConfirmDialog open={deleteIndex !== null} itemName={deleteIndex !== null ? `${amenity.name} gallery photo` : undefined} description="This photo will be removed from this amenity's public gallery." onCancel={() => setDeleteIndex(null)} onConfirm={async () => { if (deleteIndex === null) return; const saved = await saveAmenity(k, { gallery: gallery.filter((_, j) => j !== deleteIndex) }); if (!saved) { pushToast('Could not remove the photo. Please try again.', 'error'); throw new Error('Could not remove photo') } pushToast('Photo removed', 'success') }} />
    </div>
  )
}
