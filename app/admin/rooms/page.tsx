...
// A physical room has no photos, description or price of its own — it always shows its room type's.
// (rooms.image_url / rooms.images still exist in the database for older records, but nothing here
// reads or writes them any more.)
type Unit = { id:string; room_number:string; room_type_id:string; floor:string; status:string; name:string; slug:string }
...
type UnitDraft = { number:string; floor:string; name:string }
...
const emptyUnit:UnitDraft = { number:'', floor:'1', name:'' }
...
export default function RoomManagement(){
  ...
  function startEditUnit(typeId:string,r:Unit){setEditingUnit(r.id);setUnitModal(typeId);setUnit({number:r.room_number,floor:r.floor,name:r.name});setMessage(null)}
  ...
  async function saveUnit(){
    ...
    setBusy(true);setMessage(null)
    const row={room_number:unit.number.trim(),floor:unit.floor.trim()||'1',name,slug}
