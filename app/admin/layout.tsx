...
import PortalShell, { type PortalNavGroup } from '../../components/layout/PortalShell'
import {
...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const groups = ALL_GROUPS
    .map(g => ({ ...g, items: g.items.filter(item => { const s = sectionForPath(item.href); return !s || allowed(s) }) }))
    .filter(g => g.items.length > 0)

  return (
    <>
    <StoreLoader />
    <PortalShell portalName="Admin" portalTag="Hotel Management" userName={name} userRole={roleLabel} groups={groups}>
      {children}
    </PortalShell>
    </>
  )
