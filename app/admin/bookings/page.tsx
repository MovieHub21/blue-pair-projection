...
export default function BookingManagement() {
  ...
  const filtered = bookings.filter(b => { const customer = custOf(b.customerId); const query = q.toLowerCase(); return !query || [customer?.name, customer?.email, customer?.phone, b.reference].some(v => String(v ?? '').toLowerCase().includes(query)) })
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6 flex-wrap gap-3"><div><h1 className="text-2xl font-semibold">Booking Management</h1><p className="text-sm text-navy-400 mt-1">Online reservations and front-desk walk-ins in one place.</p></div><div className="flex items-center gap-2 flex-wrap"><div className="flex items-center gap-2 bg-white border border-black/10 rounded-full px-4 py-2 w-64"><Search size={14} className="text-navy-400" /><input value={q} onChange={e => setQ(e.target.value)} placeholder="Search guest, email or booking…" className="text-sm outline-none flex-1" /></div><button onClick={openWalkIn} className="btn-gold flex items-center gap-2"><UserRoundPlus size={16} /> Walk-in booking</button></div></div>
