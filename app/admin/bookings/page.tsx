...
export default function BookingManagement() {
  ...
  function openBooking(booking: typeof bookings[0]) { setActive(booking); setError(null) }
  ...
  async function createWalkIn() {
    ...
    try {
      ...
      await loadAll(); setShowWalkIn(false); setWalkIn({ ...emptyWalkIn, roomTypeId: roomTypes[0]?.id ?? '' }); setExpandedWalkInType(roomTypes[0]?.id ?? null); setWalkInAvailability([]); setError(null)
      ...
      window.setTimeout(() => { const created = useStore.getState().bookings.find(b => b.id === data.booking.id); if (created) setConfirmingPayment(created) }, 50)
