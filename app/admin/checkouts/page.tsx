...
export default function CheckOutManagement() {
  ...
  async function checkout(id: string) {
    const response = await fetch('/api/admin/checkout', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({bookingId:id}) })
    const data = await response.json().catch(()=>({}))
    if (!response.ok) { pushToast(data.error || 'Could not check out guest.','error'); return }
    checkOutBooking(id); pushToast('Guest checked out. The room is marked Cleaning Required for today.','success')
  }
