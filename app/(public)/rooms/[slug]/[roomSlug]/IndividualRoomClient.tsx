...
export default function IndividualRoomClient({ type, unit }: { type: RoomType; unit: Unit }) {
  ...
  return <div className="container-w px-6 md:px-10 py-8">
    ...
    <div className="grid lg:grid-cols-[1.4fr,.6fr] gap-10 mt-5"><div>
      ...
      <div className={`mt-5 rounded-lg px-3 py-2.5 text-sm font-semibold ${status==='available'?'bg-emerald-50 text-emerald-700':status==='availableSoon'?'bg-gold-50 text-gold-700':status==='reserved'?'bg-emerald-50 text-emerald-700':'bg-red-50 text-red-700'}`}>{checking ? 'Checking availability…' : status==='available' ? 'Available to book' : status==='availableSoon' ? `Available soon${availableFrom ? ` · from ${availableFrom}` : ''}` : status==='reserved' ? (paymentReady ? 'Available — payment ready' : 'Reserved for you') : 'Currently unavailable'}</div>
      ...
      {!checking && status==='reserved' && paymentReady && <Link href="/account/bookings" className="btn-gold w-full justify-center mt-5">Pay now <ArrowRight size={15}/></Link>}
      {!checking && status==='reserved' && !paymentReady && <div className="mt-5 rounded-xl border border-gold-200 bg-gold-50 p-4 text-xs text-gold-800 flex gap-2"><Clock3 size={15} className="shrink-0 mt-0.5"/><span>No payment yet. We’ll email you when this reserved room becomes available, then you can pay to secure it.</span></div>}
