...
import { getRoomTypes, getRoomTypeBySlug, getRoomsByType } from '../../../../lib/data'
...
export default async function RoomDetailsPage({params}:{params:{slug:string}}){
 const room=await getRoomTypeBySlug(params.slug); if(!room)notFound(); const units=await getRoomsByType(room.id); const others=(await getRoomTypes()).filter(r=>r.id!==room.id&&r.active).slice(0,3)
 ...
 return <><JsonLd data={[breadcrumbJsonLd(breadcrumbs,SITE_URL),jsonld]}/><RoomDetailsClient room={room} units={units} others={others}/></>
}
