...
function loadThree(): Promise<ThreeRuntime> {
  return new Promise((resolve, reject) => {
    ...
    if (existing) {
      if ((window as unknown as { THREE?: ThreeRuntime }).THREE) {
        resolve((window as unknown as { THREE: ThreeRuntime }).THREE)
        return
      }
      existing.addEventListener('load', () => resolve((window as unknown as { THREE: ThreeRuntime }).THREE))
    ...
    script.onload = () => {
      const THREE = (window as unknown as { THREE?: ThreeRuntime }).THREE
      if (THREE) resolve(THREE)
      else reject(new Error('Three.js global was not created'))
    }
...
function createScene(THREE: ThreeRuntime, canvas: HTMLCanvasElement) {
  ...
  const stoneDark = new THREE.MeshStandardMaterial({ color: 0x536171, metalness: 0.18, roughness: 0.52 })
  const glass = new THREE.MeshStandardMaterial({ color: 0x3b667c, metalness: 0.3, roughness: 0.16, transparent: true, opacity: 0.88 })
  const windowMat = new THREE.MeshStandardMaterial({ color: 0xf4cf78, emissive: 0xc98c35, emissiveIntensity: 0.7, roughness: 0.3 })
  ...
  const box = (w: number, h: number, d: number, material: InstanceType<ThreeRuntime['MeshStandardMaterial']>, x: number, y: number, z: number) => {
  ...
  // Main tower and two lower wings.
  box(9.4, 15, 5.6, stone, 1.5, 7.5, 0)
  box(5.4, 8.8, 6.8, stoneDark, -6.1, 4.4, 0.3)
  box(4.8, 6.8, 7.1, stoneDark, 6.9, 3.4, 0.5)

  // Recessed glass core and entrance canopy.
  ...
  box(5.8, 0.35, 4.8, stoneDark, 1.5, 15.7, 0)
  const spire = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.16, 2.2, 12), gold)
  spire.position.set(1.5, 16.8, 0)
  spire.castShadow = true
  hotel.add(spire)

  // Window grid on the main tower.
  for (let floor = 0; floor < 7; floor += 1) {
    for (let col = 0; col < 4; col += 1) {
      const x = -1.5 + col * 2.0
      const y = 4.0 + floor * 1.55
      const window = box(1.05, 0.82, 0.12, windowMat, x, y, 2.93)
      window.castShadow = false
    }
  }

  // Vertical fins make the facade feel architectural rather than blocky.
  for (let col = 0; col < 5; col += 1) {
    box(0.12, 13.4, 0.18, gold, -2.9 + col * 2.2, 7.7, 3.05)
  }
  ...
  for (let side = 0; side < 2; side += 1) {
    ...
    for (let floor = 0; floor < 4; floor += 1) {
      for (let col = 0; col < 3; col += 1) {
        box(0.85, 0.7, 0.1, windowMat, baseX - 1.3 + col * 1.3, 2.0 + floor * 1.45, side === 0 ? 3.72 : 4.12)
      }
    }
  }
  ...
  box(2.6, 0.25, 2.8, stoneDark, 6.1, 0.35, 5.1)
  ...
  // Small palm-like silhouettes using cylinders/cones.
  const palmMat = new THREE.MeshStandardMaterial({ color: 0x1b352d, roughness: 0.9 })
  for (const x of [-8.5, 9.3]) {
    const trunk = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.18, 3.2, 8), palmMat)
    trunk.position.set(x, 1.7, 5.2)
    trunk.rotation.z = x < 0 ? -0.08 : 0.08
    trunk.castShadow = true
    hotel.add(trunk)
    for (let i = 0; i < 6; i += 1) {
      const leaf = new THREE.Mesh(new THREE.ConeGeometry(0.08, 2.3, 5), palmMat)
  ...
  const ground = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x07111d, roughness: 0.82, metalness: 0.08 }),
  )
...
export default function Hotel3DHero({ className = '' }: ThreeCanvasProps) {
  ...
  return (
    <div className={`relative h-full w-full overflow-hidden ${className}`}>
      <canvas
        ref={canvasRef}
        aria-label="Interactive three-dimensional Blue Pair hotel visualization"
        className={`h-full w-full touch-none transition-opacity duration-700 ${state === 'fallback' ? 'opacity-0' : 'opacity-100'}`}
      />

      {state === 'loading' && (
        <div className="absolute inset-0 grid place-items-center">
          <div className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-[10px] uppercase tracking-[0.24em] text-white/45 backdrop-blur">
            Building the scene…
          </div>
        </div>
      )}
      ...
      {state === 'fallback' && (
        <div className="absolute inset-0 grid place-items-center p-8 text-center">
          <div className="max-w-xs rounded-2xl border border-white/10 bg-white/5 px-5 py-4 backdrop-blur-xl">
            <div className="text-[10px] uppercase tracking-[0.22em] text-[#e2bd69]">Blue Pair</div>
            <p className="mt-2 text-xs leading-relaxed text-white/50">The interactive architectural view is unavailable on this device.</p>
          </div>
        </div>
      )}

      {state === 'ready' && (
        <div className="pointer-events-none absolute bottom-5 left-1/2 -translate-x-1/2 rounded-full border border-white/10 bg-black/20 px-4 py-2 text-[9px] uppercase tracking-[0.22em] text-white/40 backdrop-blur">
