...
function createScene(canvas: HTMLCanvasElement) {
  ...
  for (let floor = 0; floor < 7; floor += 1) {
    for (let col = 0; col < 4; col += 1) {
      box(1.05, 0.82, 0.12, windows, -1.5 + col * 2, 4 + floor * 1.55, 2.93)
    }
  }
  ...
  for (const side of [-1, 1]) {
    ...
    for (let floor = 0; floor < 4; floor += 1) {
      for (let col = 0; col < 3; col += 1) {
        box(0.85, 0.7, 0.1, windows, baseX - 1.3 + col * 1.3, 2 + floor * 1.45, z)
      }
    }
  }

  const terrace = new THREE.MeshStandardMaterial({ color: 0x1b2b3b, roughness: 0.9 })
  box(15, 0.28, 7.5, terrace, 0, 0.15, 5.8)
...
export default function Hotel3DHero({ className = '' }: ThreeCanvasProps) {
  ...
  useEffect(() => {
    ...
    try {
      cleanup = createScene(canvas)
      if (!cancelled) setState('ready')
    } catch (error) {
      console.error('Blue Pair 3D architecture failed to initialize:', error)
        ...
        <div className="absolute inset-0 grid place-items-center p-8 text-center">
          <div className="max-w-xs rounded-2xl border border-white/10 bg-white/5 px-5 py-4 backdrop-blur-xl">
            ...
            <p className="mt-2 text-xs leading-relaxed text-white/50">
              The interactive architectural view is unavailable on this device.
            </p>
          </div>
        </div>
      )}
