...
type Day={date:string;status:'available'|'booked'|'held'|'occupied'|'availableSoon';reference?:string|null;source?:string|null;check_in?:string|null;check_out?:string|null}
...
export default function AvailabilityCalendar({roomId,initialCheckIn,initialCheckOut,onSelect}:{roomId:string;initialCheckIn?:string;initialCheckOut?:string;onSelect?:(checkIn:string,checkOut:string)=>void}){
 ...
 const [month,setMonth]=useState(monthStart(initialStart));const [days,setDays]=useState<Day[]>([]);const [loading,setLoading]=useState(true);const [error,setError]=useState<string|null>(null);const [start,setStart]=useState(initialStart);const [end,setEnd]=useState(initialEnd)
 useEffect(()=>{let cancelled=false;const from=month;const to=addDaysISO(1,`${month.slice(0,7)}-${pad(daysInMonth(month))}`);setLoading(true);fetch(`/api/public/room-calendar?roomId=${encodeURIComponent(roomId)}&from=${from}&to=${to}`,{cache:'no-store'}).then(r=>r.ok?r.json():null).then(data=>{if(cancelled)return;if(data?.days){setDays(data.days);setError(null)}else setError(data?.error||'Unable to load calendar.')}).catch(()=>{if(!cancelled)setError('Unable to load calendar.')}).finally(()=>{if(!cancelled)setLoading(false)});return()=>{cancelled=true}},[roomId,month])
