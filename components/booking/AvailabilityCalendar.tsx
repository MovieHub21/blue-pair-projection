...
export default function AvailabilityCalendar({roomId,initialCheckIn,initialCheckOut,onSelect}:{roomId:string;initialCheckIn?:string;initialCheckOut?:string;onSelect?:(checkIn:string,checkOut:string)=>void}){
 ...
 return (
  <div className="rounded-2xl border border-black/[0.07] bg-white p-4 sm:p-5 shadow-sm">
   ...
   <div className="flex flex-wrap gap-3 mt-4 pt-3 border-t border-black/[0.07] text-[9px] text-navy-500"><span>🟢 Available</span><span>🔴 Taken</span><span>🟡 Held</span><span>⚫ Available Soon / Unavailable</span></div>{start&&end&&<p className="text-[10px] text-navy-500 mt-3 text-center">Selected: <b>{start}</b> → <b>{end}</b></p>}
  </div>
 )
}
