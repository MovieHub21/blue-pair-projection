...
export default function AvailabilityCalendar({roomId,initialCheckIn,initialCheckOut,onSelect}:{roomId:string;initialCheckIn?:string;initialCheckOut?:string;onSelect?:(checkIn:string,checkOut:string)=>void}){
 ...
 useEffect(()=>{let cancelled=false
  const load=async()=>{const startedAt=Date.now();const from=month;const to=addDaysISO(1,`${month.slice(0,7)}-${pad(daysInMonth(month))}`);setLoading(true);try{const response=await fetch(`/api/public/room-calendar?roomId=${encodeURIComponent(roomId)}&from=${from}&to=${to}&_=${Date.now()}`,{cache:'no-store'});const data=await response.json().catch(()=>null);console.info('[room-calendar][client-response]',{roomId,from,to,ok:response.ok,status:response.status,dayCount:data?.days?.length??0,error:data?.error??null,elapsedMs:Date.now()-startedAt});if(cancelled)return;if(response.ok&&data?.days){setDays(data.days);setError(null);if(!initialCheckIn||initialCheckIn<=today){const availablePair=data.days.find((d:Day,i:number)=>d.date>today&&d.status==='available'&&data.days[i+1]?.date===addDaysISO(1,d.date)&&data.days[i+1]?.status==='available');if(availablePair){const next=addDaysISO(1,availablePair.date);setStart(availablePair.date);setEnd(next)}}}else setError(data?.error||'Unable to load calendar.')}catch{if(!cancelled)setError('Unable to load calendar.')}finally{if(!cancelled)setLoading(false)}}
  void load()
  const refresh=(event:Event)=>{const detail=(event as CustomEvent).detail||{};console.info('[room-calendar][realtime-refresh]',{roomId,month,table:detail.table??null,operation:detail.operation??null,receivedAt:new Date().toISOString()});void load()}
  window.addEventListener('bluepair:database-change',refresh)
  return()=>{cancelled=true;window.removeEventListener('bluepair:database-change',refresh);console.info('[room-calendar][effect-cleanup]',{roomId,month})}
 },[roomId,month])
