...
export default function PublicNavbar() {
  ...
  const portals = Array.from(new Map(auth.roles.map(role => [portalByRole[role]?.href, portalByRole[role]]).filter((entry): entry is [string, { href: string; label: string }] => Boolean(entry[1]))).values())
    ...
    <nav className="sticky top-0 z-40 bg-cream-50/90 backdrop-blur-md border-b border-black/5">
      <div className="container-w flex items-center justify-between py-3.5 px-5 md:px-10">
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          {auth.loading ? <div className="w-24 h-8 rounded-lg bg-black/5 animate-pulse" /> : auth.userId ? <div className="relative" onMouseEnter={() => setAccountOpen(true)} onMouseLeave={() => setAccountOpen(false)}><button className="flex items-center gap-2 text-[13.5px] font-semibold text-navy-800"><span className="w-7 h-7 rounded-full bg-navy-900 text-gold-400 text-[11px] font-bold flex items-center justify-center">{initials(firstName || 'G')}</span>{firstName || 'Account'} <ChevronDown size={14} /></button>{accountOpen && <div className="absolute top-full right-0 pt-3 w-52"><div className="card p-2 grid gap-0.5"><Link href="/account/dashboard" className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm hover:bg-cream-100"><LayoutGrid size={14} />Dashboard</Link><Link href="/account/profile" className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm hover:bg-cream-100"><User size={14} />Profile</Link>{portals.map(item => <Link key={item.href} href={item.href} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-semibold text-navy-900 hover:bg-cream-100"><ShieldCheck size={14} />{item.label}</Link>)}<button onClick={handleSignOut} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm hover:bg-cream-100 text-left text-red-600"><LogOut size={14} />Sign out</button></div></div>}</div> : <Link href="/account/login" className="text-[13.5px] font-semibold text-navy-800">Sign in</Link>}
      ...
      {open && <div className="lg:hidden px-6 pb-6 flex flex-col gap-1 text-sm font-medium max-h-[calc(100dvh-73px)] overflow-y-auto overscroll-contain">
        ...
        {auth.userId && portals.map(item => <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className="py-2.5 border-b border-black/5 flex items-center gap-2 font-semibold"><ShieldCheck size={15} />{item.label}</Link>)}
