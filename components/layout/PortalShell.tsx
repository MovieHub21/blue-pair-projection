  ...
  return (
    <div className="portal-shell min-h-screen flex">
      <div className="lg:hidden fixed top-0 inset-x-0 z-40 bg-navy-950 text-white flex items-center justify-between px-4 py-3.5">
        ...
        {menuOpen && <div className="absolute top-full right-3 pt-2 w-[calc(100vw-24px)] max-w-56 z-50"><div className="bg-white rounded-xl2 shadow-pop border border-black/5 p-2 text-navy-600"><div className="border-t border-black/5"><button onClick={signOut} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-[13px] font-medium text-red-600 hover:bg-red-50 text-left"><LogOut size={14} /> Sign out</button></div></div></div>}
      </div>
      ...
      <aside className={'fixed lg:sticky top-0 h-screen w-72 bg-navy-950 text-white/70 flex flex-col z-50 transition-transform duration-200 ' + (open ? 'translate-x-0' : '-translate-x-full lg:translate-x-0')}>
        <div className="flex items-center justify-between px-6 py-6"><div><Link href="/" className="flex items-center gap-2 font-display text-lg font-semibold text-white"><span className="w-2 h-2 rounded-full bg-gold-500" />Blue Pair</Link><span className="text-[10px] uppercase tracking-wider text-gold-400 font-bold">{portalTag}</span></div><button onClick={() => setOpen(false)} className="lg:hidden w-9 h-9 flex items-center justify-center rounded-lg hover:bg-white/10" aria-label="Close navigation"><X size={18} /></button></div>
