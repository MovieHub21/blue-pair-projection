  ...
  return (
    <div className="portal-shell min-h-screen flex">
      <div className="lg:hidden fixed top-0 inset-x-0 z-40 bg-navy-950 text-white flex items-center justify-between px-4 py-3.5">
        ...
        <span className="flex items-center gap-2 font-display text-sm font-semibold truncate max-w-[48%]"><img src="/icon-192.png" alt="" className="w-6 h-6 rounded-md object-cover" />{portalName}</span>
      ...
      <aside className={'fixed lg:sticky top-0 h-screen w-72 bg-navy-950 text-white/70 flex flex-col z-50 transition-transform duration-200 ' + (open ? 'translate-x-0' : '-translate-x-full lg:translate-x-0')}>
        <div className="flex items-center justify-between px-6 py-6"><div><Link href="/" className="flex items-center gap-2 font-display text-lg font-semibold text-white"><img src="/icon-192.png" alt="Blue Pair" className="w-8 h-8 rounded-lg object-cover" />Blue Pair</Link><span className="text-[10px] uppercase tracking-wider text-gold-400 font-bold">{portalTag}</span></div><button onClick={() => setOpen(false)} className="lg:hidden w-9 h-9 flex items-center justify-center rounded-lg hover:bg-white/10" aria-label="Close navigation"><X size={18} /></button></div>
