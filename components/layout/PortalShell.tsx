...
export default function PortalShell({ portalName, portalTag, groups, userName, userRole, showAdminPortal = false, children }: { portalName: string; portalTag: string; groups: PortalNavGroup[]; userName: string; userRole: string; showAdminPortal?: boolean; children: ReactNode }) {
  ...
  return <div className="portal-shell min-h-screen flex">
    <div className="lg:hidden fixed top-0 inset-x-0 z-40 bg-navy-950/95 text-white backdrop-blur-xl border-b border-white/10 flex items-center justify-between px-3.5 py-2.5">
      ...
      {menuOpen && <div className="absolute top-full right-3 pt-2 w-[calc(100vw-24px)] max-w-56 z-50"><div className="bg-white rounded-xl2 shadow-pop border border-black/5 p-2 text-navy-600"><a href="https://bluepairsignature.com/" onClick={() => setMenuOpen(false)} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-[13px] font-semibold text-navy-800 hover:bg-cream-100">View public site ↗</a>{showAdminPortal && isGuestPortal && <Link href="/admin/dashboard" onClick={() => setMenuOpen(false)} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-[13px] font-semibold text-navy-800 hover:bg-cream-100"><span className="w-5 h-5 rounded-md bg-gold-500/15 text-gold-700 flex items-center justify-center text-[9px]">A</span> Admin Portal</Link>}<button onClick={signOut} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-[13px] font-medium text-red-600 hover:bg-red-50 text-left"><LogOut size={14} /> Sign out</button></div></div>}
    </div>
